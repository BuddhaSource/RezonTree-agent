#!/usr/bin/env tsx
// topup-from-bob.ts — bob → eve USDC rebalancing.
//
// Battle conservation moves sponsor escrow → solver winnings each
// scenario. Bob accumulates; eve drains. This script recycles
// bob's accumulated pool back to eve so the battle can keep
// going. Total USDC across the wallet pool stays constant; only
// the role-allocation rebalances.
//
// F-NEW-6 hardening: skip the transfer if eve already has more
...
const eve = mnemonicToAccount(mnemonic, { addressIndex: 5 });
...
const eveBal = (await publicClient.readContract({
  address: USDC, abi: erc20Abi, functionName: "balanceOf", args: [eve.address],
})) as bigint;
const bobBal = (await publicClient.readContract({
  address: USDC, abi: erc20Abi, functionName: "balanceOf", args: [bob.address],
})) as bigint;

if (eveBal >= MIN_SPONSOR_BUFFER_USDC) {
  console.log(`eve has ${fmt(eveBal)} USDC (>= floor ${fmt(MIN_SPONSOR_BUFFER_USDC)}); skipping`);
  process.exit(0);
}
...
console.log(`transferring ${fmt(requested)} USDC bob → eve (eve ${fmt(eveBal)} → ${fmt(eveBal + requested)})…`);
const hash = await wallet.sendTransaction({
  account: bob, chain: baseSepolia, to: USDC,
  data: encodeFunctionData({ abi: erc20Abi, functionName: "transfer", args: [eve.address, requested] }),
});
console.log(`  tx ${hash}`);
